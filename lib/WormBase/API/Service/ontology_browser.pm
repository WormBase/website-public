...
sub index {
  ...
  foreach my $category (@ontologyCategories) {						# for each category
    ...
    foreach my $rootTerm (@{ $ontologyCategories{$category} }) {			# for each root term under the category
      ...
      my $load_widget_link = qq(<a href="#o_browser" id="load_$rootTerm" onclick="document.getElementById('o_browser').innerHTML = 'loading $rootTerm $escaped_name'; \$jq('#o_browser').load('$url');">$escaped_name ($rootTerm)</a>);
#       my $load_widget_link = qq(<a href="#o_browser" id="load_$rootTerm" onclick="document.getElementById('o_browser').innerHTML = 'loading $rootTerm $names{$rootTerm}'; \$jq('#o_browser').load('$url');">$names{"$rootTerm"} ($rootTerm)</a>);
...
sub query_children {
    ...
    foreach my $child (sort { lc($hash{$a}{name}) cmp lc($hash{$b}{name}) } keys %hash) {         # for each child sorted alphabetically case-insensitive
      ...
      my $load_widget_link = qq(<a href="#o_browser" id="load_$child" onclick="document.getElementById('o_browser').innerHTML = 'loading $child $escaped_name'; \$jq('#o_browser').load('$url');">$escaped_name ($child)</a>);  
#       my $load_widget_link = qq(<a href="#o_browser" id="load_$child" onclick="document.getElementById('o_browser').innerHTML = 'loading $child $hash{$child}{name}'; \$jq('#o_browser').load('$url');">$hash{$child}{name} ($child)</a>);  
          										# a link to load node into the o_browser div by doing a jquery load
...
sub show_genes {
    ...
    my %geneData;									# datatype -> id -> name/species
    my %sentenceData;									# sentence header for table of information per datatype
    my %filenameData;									# filename for each datatype
    ...
    foreach my $type (sort keys %url) {			# for each type of URL for the term's class
      ...
      if ($type =~ m/^infDir/) {           $sentenceData{$type} .= qq( or any of its transitive descendant terms); }
      if ($type =~ m/_rnai$/) {            $sentenceData{$type} .= qq( via RNAi); }
        elsif ($type =~ m/_variation$/) {  $sentenceData{$type} .= qq( via Variation); }

      if ( $type =~ m/_cele$/ ) {          $filenameData{$type} .= qq(elegans_); }
        elsif ($type =~ m/_noncele$/)  {   $filenameData{$type} .= qq(non_elegans_); }
      $filenameData{$type} .= qq(genes_); 
      if ($type =~ m/_rnai$/) {            $filenameData{$type} .= qq(rnai_); }
        elsif ($type =~ m/_variation$/) {  $filenameData{$type} .= qq(variation_); }
    ...
    return { 
             ...
             filenameData  => \%filenameData,		# pass the filenameData through catalyst for view
...
sub run {
    ...
    for my $index (0 .. @nodes) {					# for each node, add to graph
      ...
      if (($class eq 'disease') || ($class eq 'do_term')) { 
        $url = "/resources/disease/$id"; }	# URL to link to wormbase page for disease class
...
#       my $genes_exclusively_url = 'http://caltech-curation.textpressolab.com/~azurebrd/var/work/raymond/table_anatomy_to_genes';	# source of mappings
...
sub makeUrl {								# generate URL to WormBase object given a class and focus term id
  ...
  if (($class eq 'disease') || ($class eq 'do_term')) { 		# disease class has a special URL
    $url = "/resources/disease/$focusTermId"; }
  return $url;
} # sub makeUrl
