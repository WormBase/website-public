...
sub get_object {
  ...
  $self->log->info($self . "::get_object(): class:$class name:$name");
  
  my $db = $self->dbh();
  my $formatted_class = ucfirst($class);
  my $object = $db->fetch(-class=>$formatted_class,-name=>$name,-fill=>1);  
  
  # It's a little goofy to stash the version here.
  # Should probably just do it once at connect
  # config or stash?
  #  $c->config->{ace_version} = $db->version;
  #  $c->config->{ace_date}    = $db->date;
  return $object;
}
