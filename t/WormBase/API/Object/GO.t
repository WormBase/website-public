...
my $GO_term = 'GO:0032502';

my $go_obj = $wormbase->fetch({class=>'GO_Term',name=>$GO_term}); 
isa_ok($go_obj,'WormBase::API::Object::GO_Term');
...
foreach my $tag (@tags) {
	...
	print "\n#######\n";
	note("Testing get_tag_data() for $tag...");
	my $result = test_get_tag_data($tag);
	print_data($result);
}
...
sub test_method {
    my $method = shift;
    my $result = $go_obj->$method;
    ok($result, "$method() worked fine");
    return $result;
}

sub test_get_tag_data {
	my $tag = shift;
	my $result = $go_obj->get_tag_data($tag);
	ok($result, "get_tag_data() worked fine for $tag");
	return $result;
