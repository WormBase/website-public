...
var Scrolling = (function(){
  ...
  var openid = {
    ...
    function setupCytoscapeGoGraph(elements){
      Plugin.getPlugin('cytoscape_js',function(){
        loadCytoscapeForGoGraph(elements)
        return;
      });
    }

    function loadCytoscapeForLifestageGraph(elements) {
      Plugin.getPlugin('qtip', function() {
        Plugin.getPlugin('cytoscape_js_qtip', function() {
          Plugin.getPlugin('cytoscape_js_dagre', function() {
            setupCyLifestageGraph(elements);
          });
        });
      });
    }
    ...
    function setupCytoscapeLifestageGraph(elements){
      Plugin.getPlugin('cytoscape_js',function(){
        loadCytoscapeForLifestageGraph(elements)
        return;
      });
    }

    function loadCytoscapeForAnatomyGraph(elements) {
      Plugin.getPlugin('qtip', function() {
        Plugin.getPlugin('cytoscape_js_qtip', function() {
          Plugin.getPlugin('cytoscape_js_dagre', function() {
            setupCyAnatomyGraph(elements);
          });
        });
      });
    }
    ...
    function setupCytoscapeAnatomyGraph(elements){
      Plugin.getPlugin('cytoscape_js',function(){
        loadCytoscapeForAnatomyGraph(elements)
        return;
      });
    }

    function loadCytoscapeForDiseaseGraph(elements) {
      Plugin.getPlugin('qtip', function() {
        Plugin.getPlugin('cytoscape_js_qtip', function() {
          Plugin.getPlugin('cytoscape_js_dagre', function() {
            setupCyDiseaseGraph(elements);
          });
        });
      });
    }
    ...
    function setupCytoscapeDiseaseGraph(elements){
      Plugin.getPlugin('cytoscape_js',function(){
        loadCytoscapeForDiseaseGraph(elements)
        return;
      });
    }

    function loadCytoscapeForPhenotypeGraph(elements) {
      Plugin.getPlugin('qtip', function() {
        Plugin.getPlugin('cytoscape_js_qtip', function() {
          Plugin.getPlugin('cytoscape_js_dagre', function() {
            setupCyPhenotypeGraph(elements);
          });
        });
      });
    }
    ...
    function setupCytoscapePhenotypeGraph(elements){
      Plugin.getPlugin('cytoscape_js',function(){
        loadCytoscapeForPhenotypeGraph(elements)
        return;
      });
    }
    ...
    function BACKUPsetupCyLifestageGraph(elements){
console.log('lifestage ' + elements.nodes[0].data.id);
      var cyLifestageGraph = window.cyLifestageGraph = cytoscape({
        container: document.getElementById('cyLifestageGraph'),
        ...
        ready: function(){
          window.cyLifestageGraph = this;
          cyLifestageGraph.elements().unselectify();
          $jq('#cyLifestageGraphLoading').hide();

          cyLifestageGraph.on('tap', 'node', function(e){
            var node = e.cyTarget;
            var nodeId   = node.data('id');
            var neighborhood = node.neighborhood().add(node);
            cyLifestageGraph.elements().addClass('faded');
            neighborhood.removeClass('faded');
          ...
          cyLifestageGraph.on('tap', function(e){
            if( e.cyTarget === cyLifestageGraph ){
              cyLifestageGraph.elements().removeClass('faded');
            }
          });

          cyLifestageGraph.on('mouseover', 'node', function(event) {
      ...
      $jq('#radio_weighted').on('click', function(){
        var nodes = cyLifestageGraph.nodes();
        for( var i = 0; i < nodes.length; i++ ){
          var node     = nodes[i];
          var nodeId   = node.data('id');
          var diameterWeighted   = node.data('diameter_weighted');
          cyLifestageGraph.$('#' + nodeId).data('diameter', diameterWeighted);
          var fontSizeWeighted   = node.data('fontSizeWeighted');
          cyLifestageGraph.$('#' + nodeId).data('fontSize', fontSizeWeighted);
          var borderWidthWeighted   = node.data('borderWidthWeighted');
          cyLifestageGraph.$('#' + nodeId).data('borderWidth', borderWidthWeighted);
        }
        cyLifestageGraph.layout();
      });
      $jq('#radio_unweighted').on('click', function(){
        ...
        for( var i = 0; i < nodes.length; i++ ){
          ...
          var diameterWeighted   = node.data('diameter_weighted');
          cyLifestageGraph.$('#' + nodeId).data('diameter', diameterUnweighted);
          var fontSizeUnweighted = node.data('fontSizeUnweighted');
          var fontSizeWeighted   = node.data('fontSizeWeighted');
          cyLifestageGraph.$('#' + nodeId).data('fontSize', fontSizeUnweighted);
          var borderWidthUnweighted = node.data('borderWidthUnweighted');
          var borderWidthWeighted   = node.data('borderWidthWeighted');
          cyLifestageGraph.$('#' + nodeId).data('borderWidth', borderWidthUnweighted);
        }
        cyLifestageGraph.layout();
      });
      $jq('#view_png_button').on('click', function(){
        var png64 = cyLifestageGraph.png({ full: true, maxWidth: 8000, maxHeight: 8000, bg: 'white' });
        $jq('#png-export').attr('src', png64);
        $jq('#png-export').show();
        $jq('#cyLifestageGraph').hide();
        $jq('#weightstate').hide();
    ...
    function BACKUPsetupCyAnatomyGraph(elements){
console.log('anatomy ' + elements.nodes[0].data.id);
      var cyAnatomyGraph = window.cyAnatomyGraph = cytoscape({
        container: document.getElementById('cyAnatomyGraph'),
        layout: { name: 'dagre', padding: 10, nodeSep: 5 },
        style: cytoscape.stylesheet()
          .selector('node')
            .css({
        ...
        ready: function(){
          window.cyAnatomyGraph = this;
          cyAnatomyGraph.elements().unselectify();
          $jq('#cyAnatomyGraphLoading').hide();

          cyAnatomyGraph.on('tap', 'node', function(e){
            var node = e.cyTarget;
            var nodeId   = node.data('id');
            var neighborhood = node.neighborhood().add(node);
            cyAnatomyGraph.elements().addClass('faded');
            neighborhood.removeClass('faded');
          ...
          cyAnatomyGraph.on('tap', function(e){
            if( e.cyTarget === cyAnatomyGraph ){
              cyAnatomyGraph.elements().removeClass('faded');
            }
          });

          cyAnatomyGraph.on('mouseover', 'node', function(event) {
              var node = event.cyTarget;
              var nodeId   = node.data('id');
              var nodeName = node.data('name');
              var annotCounts = node.data('annotCounts');
      ...
      $jq('#radio_weighted').on('click', function(){
        var nodes = cyAnatomyGraph.nodes();
        for( var i = 0; i < nodes.length; i++ ){
          var node     = nodes[i];
          var nodeId   = node.data('id');
          var diameterWeighted   = node.data('diameter_weighted');
          cyAnatomyGraph.$('#' + nodeId).data('diameter', diameterWeighted);
          var fontSizeWeighted   = node.data('fontSizeWeighted');
          cyAnatomyGraph.$('#' + nodeId).data('fontSize', fontSizeWeighted);
          var borderWidthWeighted   = node.data('borderWidthWeighted');
          cyAnatomyGraph.$('#' + nodeId).data('borderWidth', borderWidthWeighted);
        }
        cyAnatomyGraph.layout();
      });
      $jq('#radio_unweighted').on('click', function(){
        var nodes = cyAnatomyGraph.nodes();
        for( var i = 0; i < nodes.length; i++ ){
          ...
          var diameterUnweighted = node.data('diameter_unweighted');
          var diameterWeighted   = node.data('diameter_weighted');
          cyAnatomyGraph.$('#' + nodeId).data('diameter', diameterUnweighted);
          var fontSizeUnweighted = node.data('fontSizeUnweighted');
          var fontSizeWeighted   = node.data('fontSizeWeighted');
          cyAnatomyGraph.$('#' + nodeId).data('fontSize', fontSizeUnweighted);
          var borderWidthUnweighted = node.data('borderWidthUnweighted');
          var borderWidthWeighted   = node.data('borderWidthWeighted');
          cyAnatomyGraph.$('#' + nodeId).data('borderWidth', borderWidthUnweighted);
        }
        cyAnatomyGraph.layout();
      });
      $jq('#view_png_button').on('click', function(){
        var png64 = cyAnatomyGraph.png({ full: true, maxWidth: 8000, maxHeight: 8000, bg: 'white' });
        $jq('#png-export').attr('src', png64);
        $jq('#png-export').show();
        $jq('#cyAnatomyGraph').hide();
        $jq('#weightstate').hide();
        $jq('#view_png_button').hide();
        $jq('#view_edit_button').show();
        $jq('#info').text('drag image to desktop, or right-click and save image as');
    ...
    function linkFromEdge(nodeObjId, datatype) {            // generate url from datatype + edge's target node's objId
      var linkout = '';
    ...
    function linkFromNode(nodeId, datatype) {               // generate url from datatype + nodeId
    ...
    function setupCyDiseaseGraph(elements){
      setupCyOntologyGraph(elements, 'Disease');
    }

    function setupCyAnatomyGraph(elements){
      setupCyOntologyGraph(elements, 'Anatomy');
    }

    function setupCyGoGraph(elements){
      setupCyOntologyGraph(elements, 'Go');
    }

    function setupCyLifestageGraph(elements){
      setupCyOntologyGraph(elements, 'Lifestage');
    }

    function setupCyPhenotypeGraph(elements){
    ...
    function setupCyOntologyGraph(elements, datatype){
      ...
      var loadingElement            = document.getElementById('cy' + datatype + 'GraphLoading'); 
      var controlsElement           = document.getElementById('controlsdiv' + datatype); 
      var infoElement               = document.getElementById('info' + datatype);   
      var radioWeightedElement      = document.getElementById('radioWeighted' + datatype);   
      var radioUnweightedElement    = document.getElementById('radioUnweighted' + datatype);   
      ...
      var viewPngButtonElement      = document.getElementById('viewPngButton' + datatype);   
      var viewEditButtonElement     = document.getElementById('viewEditButton' + datatype);   
...
console.log(datatype + ' ' + elements.nodes[0].data.id);
      var cyOntologyGraph = window.cyOntologyGraph = cytoscape({
...
console.log('clicked '+ radioUnweightedElement);
        for( var i = 0; i < nodes.length; i++ ){
          ...
          var borderWidthUnweighted = node.data('borderWidthUnweighted');
          var borderWidthWeighted   = node.data('borderWidthWeighted');
          cyOntologyGraph.$('#' + nodeId).data('borderWidth', borderWidthUnweighted);
        }
        cyOntologyGraph.layout();
      });
      $jq(viewPngButtonElement).on('click', function(){
        var png64 = cyOntologyGraph.png({ full: true, maxWidth: 8000, maxHeight: 8000, bg: 'white' });
        ...
        $jq(infoElement).text('drag image to desktop, or right-click and save image as');
      });
...
//         var rootsChosenGroup = rootsChosen.join(',');
           ...
           if (event.data.name === 'maxDepthChange') {
             if (maxDepthElement.value) { maxDepth = maxDepthElement.value; } }

           var urlBase = ''; if (urlBaseElement.value) { urlBase = urlBaseElement.value; }
...
//            var url = 'https://wobr2.caltech.edu/~raymond/cgi-bin/soba_biggo.cgi?action=annotSummaryJsonp&focusTermId=' + focusTermIdElement.value + '&datatype=' + datatype + '&radio_etl=&showControlsFlag=0&fakeRootFlag=0&filterForLcaFlag=1&filterLongestFlag=1&maxNodes=0&maxDepth=' + maxDepth;
           var url = urlBase + '&showControlsFlag=0&fakeRootFlag=0&filterForLcaFlag=1&filterLongestFlag=1&maxNodes=0&maxDepth=' + maxDepth;
...
console.log('url ' + url);
        ...
        function newCy( then ){
          ...
          $jq(loadingElement).hide(); $jq(controlsElement).show();
          cyOntologyGraph.json( { elements: elementsNew } );
          cyOntologyGraph.elements().layout({ name: 'dagre', padding: 10, nodeSep: 5 });
          ...
          maxDepthElement.options.length = 0;
          for( var i = 1; i <= maxOption; i++ ){ maxDepthElement.options[i-1] = new Option(i, i, true, false) }
          maxDepthElement.selectedIndex = maxOption - 1;
          if (event.data.name === 'maxDepthChange') { maxDepthElement.value = userSelectedValue; }
//           if (userSelectedValue <= maxDepthElement.value) { maxDepthElement.value = userSelectedValue; }
        }
      } // function updateElements()
    } // function setupCyOntologyGraph(elements, datatype)

    function BACKUPsetupCyPhenotypeGraph(elements){
console.log('phenotype ' + elements.nodes[0].data.id);
