  ...
  var WB = (function () {
    ...
    function init() {
      var pageInfo = $jq("#header").data("page"),
        ...
        history_on = (pageInfo && pageInfo['history'] === '1') ? 1 : undefined;
      if (history_on) {
      ...
      search_change(pageInfo && pageInfo['class']);
      ...
      updateCounts(pageInfo && pageInfo['ref']);
      if (pageInfo && pageInfo['notify']) { displayNotification(pageInfo['notify']); }
    ...
    var comment = {
      init: function (pageInfo) {
        comment.url = pageInfo && pageInfo['ref'];
      },
      submit: function (cm) {
    ...
    var issue = {
      init: function (pageInfo) {
        issue.url = pageInfo && pageInfo['ref'];
      },
      submit: function (is) {
        var rel = is.attr("rel"),
          url = is.attr("url"),
