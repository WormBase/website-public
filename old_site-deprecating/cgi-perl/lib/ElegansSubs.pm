# -*- Mode: perl -*-
# $Id: ElegansSubs.pm,v 1.422 2008/01/21 15:30:05 todd Exp $
# file: ElegansSubs.pm
...
use CGI qw(:standard *table *Tr *TR *td -no_xhtml center *center escape);
use CGI::Cache;
use CGI::Carp;
...
%EXPORT_TAGS = ('biblio'  => [qw(CGC ABSTRACT_INFO PrintRefs)],
		...
		'locus'   => [qw(GENE_INFO
                                 ALLELE_INFO MAP_REPORT MAP_DISPLAY STRAIN_INFO
#++                              AlleleDescription
				MutantPhenotype)]
		 );
...
sub OpenPageGetObject {
  ...
  if (@objs > 1 && !wantarray) {
    ...
    PrintTop($alternative_title || "$classname Display Page");
  }
...
sub Footer {
  ...
  return join '',
    ...
    qq{<script src="http://www.google-analytics.com/urchin.js" type="text/javascript"></script><script type="text/javascript">_uacct = "UA-2587143-$value";  _udn="wormbase.org"; urchinTracker();</script>},
...
sub GBrowseLink {
  ...
  return a({-href=>"/db/seq/gbrowse/wormbase?ref=$ref;start=$start;stop=$stop"},
	   "$refname:$s..$e");
}
...
sub PrintRefs {
    ...
    my @cgc = grep { $_->CGC_name || $_->PMID || $_->Medline_name} @$refs;
    my @wbg = grep { $_->WBG_abstract } @$refs;
...
sub Banner {
  ...
  my $searchpage = $page =~ m!^/db/searches! ||$page =~ m!^/seq/gbrowse/!;
...
#	  $url ||= 'http://www.wormbase.org/Multi/martview';
  ...
  return join '',$javascript,$return_table,$banner_image,$warning;
}
...
<script src="/gbrowse/js/x_cook.js" type="text/javascript"></script>
END
}
	...
	foreach my $paper ($type->col) {
	  ...
	  if ($paper->PMID) {
	    push (@cell,
		  ...
		  a({-href=>Configuration->Pubmed_retrieve . $paper->PMID,-target=>'_blank'},
...
sub _format_paper_evidence {
	...
	if ($paper->PMID) {
	    ...
	    push @papers,a({-href=>Object2URL($paper)},$citation);
	}
    }
    return @papers;
}
...
#++ ##  open DEBUG,">>/usr/local/wormbase/logs/debug" if ($protein eq 'WP:CE00237');
#++     for my $hit (@pep_homol) {
#++ 	next if $hit eq $protein;         # Ignore self hits
#++ 	my ($method,$score) = $hit->row(1) or next;
#++ 	
#++ ##      if ($protein eq 'WP:CE00237') {
#++ ##        print DEBUG "best: ",join(' ',$method,$best{$method}{score},$best{$method}{hit},$hit->Species),"\n";
#++ ##      }
#++ 	
#++ 
...
#++ 	
#++ ##      if ($protein eq 'WP:CE00237') {
#++ ##        print DEBUG "curr: $method $score $hit " . $hit->Species . "\n";
#++ ##        print DEBUG "now: ",join(' ',$method,$best{$method}{score},$best{$method}{hit},$hit->Species),"\n";  
#++ ##      }
#++     }
#++     
#++     foreach (values %best) {
...
#++ 	$_->{covered} = $covered;
#++ ##    print DEBUG "covered: " .  $_->{hit}->Gene_name . " $covered\n" if ($protein eq 'WP:CE00237');
#++     }
#++     
...
#++ 	    }
#++ 
#++ 	    # 2007.01 - Deprecated.  FlyBase is now using FBGn IDs, which we have in the DB - TH.
#++ #	    # hack for converting CG numbers to FBan numbers, this format needs to be confirmed -9Jul04/PC
#++ #	    if ($method =~ /fly/) { $accession =~ s/-[\w\-]+$//; 
#++ #				    $accession =~ s/^CG//i; $accession = sprintf("%07d",
#++ #										 $accession);
#++ #				}
...
#++ 	    if ($species =~ /remanei/) {
#++ 		$accession =~ s/^RP://;
#++ 		$hit = a({-href=>"http://dev.wormbase.org/db/seq/gbrowse/remanei/?name=$accession"},$accession);
#++ 		$hit .= br . i('Note: <b>C. remanei</b> predictions are based on an early assembly of the genome. Predictions subject to possibly dramatic revision pending final assembly. Sequences available on the <a href="ftp://ftp.wormbase.org/pub/wormbase/genomes/remanei">WormBase FTP site</a>.');
#++ 	    } else {
#++ 		$hit = a({-href=>$url,-target=>'_blank'},$hit);
#++ 	    }
#++ 	}
#++ 	$table .= TR(th({-align=>'left'},$species),
#++ 		     td($hit),
#++ 		     td($description),
...
sub format_reference {
  ...
  # Add a pubmed link if necessary
  my $pubmed;
  if ($paper->PMID && $pubmed_link) {
    if ($pubmed_link eq 'generic') {
      $pubmed = a({-href=>Configuration->Pubmed_retrieve . $paper->PMID},'[PubMed]');
    } elsif ($pubmed_link eq 'id') {
      $pubmed = a({-href=>Configuration->Pubmed_retrieve . $paper->PMID},"[PubMed-" . $paper->PMID . "]");
    } elsif ($pubmed_link eq 'image') {
      $pubmed = a({-href=>Configuration->Pubmed_retrieve . $paper->PMID,-target=>'_blank'},
...
#  }
  ...
  if ($curator) {
    my @temp = (
		($paper->PMID) ? 
		a({-href=>Configuration->Pubmed_retrieve . $paper->PMID},"[PubMed-" . $paper->PMID . "]") : '',
		($paper->CGC_name)
		? a({-href=>Object2URL($paper->CGC_name,'paper')},"[" . $paper->CGC_name . "]") : '',
...
sub build_citation {
    ...
    my $authors;
    if ($format eq 'long') {
	my @authors   = map { a({-href=>Object2URL($_)},$_)} $paper->Author;
	...
	if ($include_images) {
	    ...
	    # Link to PMID
	    push @links,a({-href=>Configuration->Pubmed_retrieve . $paper->PMID},
			  img({-src=>'/images/pubmed_button.gif'})) if $paper->PMID;
...
#++ 	@filtered = grep { $_->WBG_abstract } @$refs;
#++     } elsif ($category eq 'CGC') {
#++ 	@filtered = grep { $_->CGC_name || $_->PMID || $_->Medline_name} @$refs;
#++     } elsif ($category eq 'WMA') {
#++ 	@filtered = grep { $_->Meeting_abstract } @$refs;
#++     } elsif ($category eq 'WB') {
#++ 	# Blech - the type tag of Paper objects was changed unexpectedly
#++ 	@filtered = grep {$_->Remark =~ /.*WormBook.*/i} @$refs;
#++     }
#++     return @filtered;
#++ }
...
#++ ########################################
#++ Migrated to WormBase::Model::_cross_reference_remarks
#++ This is more a function of the view...
#++ sub MutantPhenotype {
#++   my $gene = shift;
#++   # fetch the description (phenotype) lines
#++   my @description = $gene->Phenotype or return;
#++ 
#++   my @xref = $gene->Allele;
...
#++ 
#++ 
#++  # MIGRATED TO WormBase::Model (And might move entirely to the View)
#++ 
...
#++ 			     td(
#++ 				join(br,
#++ 				     map {ObjectLink($_->Standard_name,undef,'_blank')} @sources)));
...
#++ 		$table .= TR(td($formatted_tag),
#++ 			     td(join(br,@sources)),
#++ 			     td(map { GetEvidenceNew(-object       => $_,
#++ 						     -format       => 'inline',
#++ 						     -display_label => 1,
#++ 						     -link_tag      => 0)} @sources ));
#++ 		
...
#++     
#++     SubSection('',b('The following phenotypes were observed in this experiment'),
#++ 	       FormatPhenotypeHash(-data      => $positives)) if @$positives > 0;
#++     
#++     SubSection('',
#++ 	       (@$positives > 0) ?  hr : '',
#++ 	       b('The following phenotypes were not observed in this experiment'),
...
sub setup_cache {
  ...
  CGI::Cache::setup({
 		     cache_options => { 
				       ...
				       max_size           => 100_000_000,  # 100 meg cache
				      }
		    }
		   );
}
