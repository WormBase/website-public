use CGI ':standard';
use Ace 1.81;
#$Id: elegans.pm,v 1.283 2007/12/19 14:47:31 todd Exp $
#$Date: 2007/12/19 14:47:31 $
#$Revision: 1.283 $

# here's the root of all our stuff
$WORMBASE = '/usr/local/wormbase';  # location of our stuff on filesystem
...
@DBGFF    = (-adaptor     => 'dbi::mysqlace',
	     -dsn         => "dbi:mysql:database=elegans;host=$MYSQL_HOST",
...
@REMGFF    = (-adaptor     => 'dbi::mysqlace',
	     -dsn         => "dbi:mysql:database=remanei;host=$MYSQL_HOST",
             -aggregators => ['processed_transcript{coding_exon,5_UTR,3_UTR/CDS}',
			      'full_transcript{coding_exon,five_prime_UTR,three_prime_UTR/Transcript}',
			      'transposon{coding_exon,five_prime_UTR,three_prime_UTR}',
			      'clone',
...
@BRIGGFF    = (-adaptor     => 'dbi::mysqlace',
	       -dsn         => "dbi:mysql:database=briggsae;host=$MYSQL_HOST",
...
@PMAPGFF    = (-adaptor     => 'dbi::mysqlace',
	     -dsn         => "dbi:mysql:database=elegans_pmap;host=$MYSQL_HOST",
...
@GMAPGFF    = (-adaptor     => 'dbi::mysqlace',
	       -dsn         => "dbi:mysql:database=elegans_gmap;host=$MYSQL_HOST",
	       -aggregators => [qw(wormbase_gene)],
	       -user        => $MYSQL_USER || 'nobody',
	       -pass        => $MYSQL_PASS || '',
	       );

$GMAPGFF_URL  = '/db/seq/gbrowse/elegans_gmap/?name=%s';
...
%BLAT_DB_DIRS      = (
					'Elegans'           => '/usr/local/wormbase/databases/blat/c_elegans',
					'Briggsae_genomic'  => '/usr/local/wormbase/databases/blat/c_briggsae'
                    );
...
$MY_FOOTER = 
# the call to comment() was causing random crashes -- I don't know where it is
# supposed to be defined, but it isn't always loaded.
#  comment('WormBase Footer') .
  CGI::table({-width=>'100%'},
	CGI::TR(CGI::td({-align=>'left',-class=>'small'},
	      CGI::a({href=>'mailto:webmaster@wormbase.org'},
		'webmaster@www.wormbase.org')),
	   CGI::td({-align=>'right',-class=>'small'},
	      CGI::a({-href=>"/copyright.html"},'Copyright Statement'))),
	CGI::TR(CGI::td({-align=>'left',-class=>'small'},
	      CGI::a({-href=>"/db/misc/feedback",-target=>'_blank'},
		'Send comments or questions to WormBase')),
	   CGI::td({-align=>'right',-class=>'small'},
	      CGI::a({-href=>"/privacy.html"},'Privacy Statement'))));
...
#			     label => 'GO Term'},
	     ...
	     hunter     => { url   => "$ROOT/seq/gbrowse/elegans/",
...
#    if ($display eq 'hunter') {
#	my $genus_species = $name->Species;
#	my ($source) = $genus_species =~ /.* (.*)/;
#    }
...
#	return sprintf("http://srs.ebi.ac.uk/srs6bin/cgi-bin/wgetz?-id+3LIc21HeBHW+-e+[SWALL:'%s']",$1);
    ...
    if ($class eq 'Homol_data' && ref $name) {
      ...
      return "/db/seq/gbrowse?name=$ref;source=wormbase";
    }
...
$FEEDBACK_RECIPIENT = 'wormbase-help@wormbase.org';
...
$HUNTER = "/db/seq/gbrowse/elegans/?name=";
...
# external URLs
$WIKI         = 'http://www.wormbase.org/wiki/index.php/%s';
...
$WB_NOMENCLATURE            = 'http://www.wormbase.org/wiki/index.php/Nomenclature';
