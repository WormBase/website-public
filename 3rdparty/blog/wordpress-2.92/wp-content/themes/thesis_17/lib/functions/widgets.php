...
function thesis_widget_recent_entries_register() {
	...
	$widget_ops = array('classname' => 'widget_killer_recent_entries', 'description' => __('Show recent posts from any category or your entire site', 'thesis'));
	$control_ops = array('width' => '', 'height' => '', 'id_base' => 'widget_killer_recent_entries');
	$name = __('Killer Recent Entries', 'thesis');
	$id = false;
...
function thesis_default_widget($sidebar = 1) {
	...
	if ((!dynamic_sidebar($sidebar) && $thesis_site->display['sidebars']['default_widgets']) || $_GET['template']) {
?>
					<li class="widget">
