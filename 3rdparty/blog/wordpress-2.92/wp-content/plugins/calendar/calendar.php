	...
	<form name="quoteform" id="quoteform" class="wrap" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=calendar">
		...
		<div id="linkadvanceddiv" class="postbox">
			<div style="float: left; width: 98%; clear: both;" class="inside">
				...
				<td><input type="text" name="event_title" class="input" size="40" maxlength="50"
...
if ( $action == 'add' )
{
	...
	// The title must be at least one character in length and no more than 100 - no non-standard characters allowed
//	if (preg_match('/^[a-zA-Z0-9]{1}[a-zA-Z0-9[:space:]]{0,99}$/',$title))
//        {
	    $title_ok =1;
//	  }
//    else
...
function edit_calendar_config()
{
  ...
  <form name="quoteform" id="quoteform" class="wrap" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=calendar-config">
				...
				<td><legend><?php _e('Display upcoming events?','calendar'); ?></legend></td>
				    ...
				    <?php _e('for','calendar'); ?> <input type="text" name="display_upcoming_days" value="<?php echo $upcoming_days ?>" size="1" maxlength="3" /> <?php _e('days into the future','calendar'); ?>
                                </td>
                                </tr>
                                <tr>
