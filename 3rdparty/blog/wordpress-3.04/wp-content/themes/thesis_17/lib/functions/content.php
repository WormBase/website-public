...
function thesis_show_byline() {
	...
	if (is_page()) {
		if ($thesis_site->display['byline']['page']['author'] || $thesis_site->display['byline']['page']['date'] || ($thesis_site->display['byline']['num_comments']['show'] && comments_open() && !$thesis_site->comments['disable_pages']))
			return true;
	}
	else {
		if ($thesis_site->display['byline']['author']['show'] || $thesis_site->display['byline']['date']['show'] || ($thesis_site->display['byline']['num_comments']['show'] && (comments_open() || get_comments_number() > 0)))
			return true;
	}
}
...
function thesis_post_categories() {
	...
	if ($thesis_site->display['byline']['categories']['show']) {
?>
...
function thesis_post_tags() {
	...
	if ((is_single() && $thesis_site->display['tags']['single']) || (!is_single() && $thesis_site->display['tags']['index'])) {
		...
		if ($post_tags) {
			...
			if ($thesis_site->display['tags']['nofollow'])
				$nofollow = ' nofollow';
?>
...
function thesis_read_more_text() {
	...
	if ($read_more != '')
		return $read_more;
	elseif ($thesis_site->display['posts']['read_more_text'])
		return urldecode($thesis_site->display['posts']['read_more_text']);
	else
