...
require('../../root/js/jquery/plugins/dataTables/media/css/demo_table.css');

var Root = require('./components/Root').default;

var SingleWidgetPage = require('./components/SingleWidgetPage').default;

var name2widget = {
  references: require('./components/widgets/shared/references').default,
};
  ...
  var WB = (function() {
    ...
    function init() {
      var pageInfo = $jq('#header').data('page'),
        ...
        sysMessage = $jq('#top-system-message').children(
          '.system-message-close'
        ),
        history_on = pageInfo['history'] === '1' ? 1 : undefined;
      if (history_on) {
        $jq.post('/rest/history', {
          ref: pageInfo['ref'],
          name: pageInfo['name'],
          id: pageInfo['id'],
          class: pageInfo['class'],
          type: pageInfo['type'],
          is_obj: pageInfo['is_obj'],
        });
      }
      ...
      if (sysMessage.size() > 0) {
        ...
        sysMessage.click(function() {
          systemMessage('hide', sysMessage.data('id'));
        });
      }
      ...
      if (pageInfo['notify']) {
        displayNotification(pageInfo['notify']);
      }
      ...
      if (searchAll.size() > 0) {
        ...
        if ($jq('.star-status-' + pageInfo['wbid']).size() > 0) {
          $jq('.star-status-' + pageInfo['wbid']).load(
            '/rest/workbench/star?wbid=' +
              pageInfo['wbid'] +
              '&name=' +
              pageInfo['name'] +
              '&class=' +
    ...
    function histUpdate(history_on) {
      ...
      if (uhc.size() > 0 && uhc.text().length > 4)
        ajaxGet(uhc, '/rest/history', { cache: false });
      if (history_on) {
    ...
    function ajaxError(xhr) {
      var error =
          (xhr.responseText &&
            $jq(xhr.responseText.trim())
              .find('.error-message-technical')
              .html()) ||
          '',
        statusText =
          xhr.statusText === 'timeout' && xhr.requestURL
            ? 'timeout: <a href="' +
              xhr.requestURL +
              '" target="_blank">try going to the widget directly</a>'
            : xhr.statusText;
      return (
        '<div class="ui-state-error ui-corner-all"><p><strong>Sorry!</strong> An error has occured.</p>' +
        '<p><a href="/tools/support?url=' +
        location.pathname +
        (error
          ? '&msg=' + encodeURIComponent(error.replace(/^\s+|\s+$|\n/gm, ''))
          : '') +
        '"><button class="ui-state-active"><span>Let us know</span></button></a></p><p>' +
        error +
        '</p><p>' +
        statusText +
        '</p></div>'
      ...
      ajaxGet($jq('.status-bar'), '/rest/auth', { cache: false }, function() {
        ...
        if (login.size() > 0) {
          login.click(function() {
            $jq(this)
              .toggleClass('open ui-corner-top')
              .siblings()
              .toggle();
          });
        } else {
    ...
    function pageInit() {
      ...
      $jq('#nav-min').click(function() {
        ...
        $jq(this)
          .attr('title', msg)
          .children('#nav-min-icon')
          .toggleClass('ui-icon-triangle-1-w ui-icon-triangle-1-e');
        Layout.updateLayout();
      ...
      if (personSearch.size() > 0) {
        ajaxGet(personSearch, personSearch.attr('href'), undefined, function() {
          ...
          personSearch.delegate(
            '.results-person .result li a',
            'click',
            function() {
              ...
              $jq.ajax({
                ...
                success: function(data) {
                  ...
                  if (linkAccount.size() === 0) {
                    ...
                    $jq('.register-notice')
                      .html("<span id='fade'>" + data.message + '</span>')
                      .show();
          ...
          if (content.text().length < 4) {
            ...
            if (lWidth >= 90) {
              ...
              var leftHeight = height(
                widgetHolder.children('.left').children('.visible')
              );
              rightHeight = height(
                widgetHolder.children('.right').children('.visible')
              );
      ...
      widgetHolder.find('.module-max').click(function() {
        var module = $jq(this).parents('.widget-container'),
          ...
          popout = window.open(
            '',
            'test',
            'height=' + module.height() + ',width=' + module.width()
          );
        ...
        if (tog.hasClass('load-toggle')) {
          if (tog.attr('iframe')) {
            ...
            iframeContainer.html(
              '<iframe src="' + tog.attr('href') + '"></iframe>'
            );
          } else {
      ...
      content.delegate('.bench-update', 'click', function() {
        var update = $jq(this),
          ...
          url =
            update.attr('ref') +
            '?name=' +
            escape(update.attr('name')) +
            '&url=' +
            escape(update.attr('href')) +
            '&save_to=' +
            save_to +
            '&is_obj=' +
            update.attr('is_obj');
        $jq('.star-status-' + wbid)
          .find('#save')
          .toggleClass('ui-icon-star-yellow ui-icon-star-gray');
    ...
    function moduleMin(button, hover, direction, callback) {
      ...
      if (direction && button.attr('title') !== direction) {
        if (callback) {
          callback();
        }
        return;
      }
      module.slideToggle('fast', function() {
        Scrolling.sidebarMove();
        if (callback) {
          callback();
        }
      ...
      !button.hasClass('show')
        ? button.attr('title', 'maximize').addClass('show')
        : button.attr('title', 'minimize').removeClass('show');
    ...
    function systemMessage(action, messageId) {
      ...
      if (action === 'show') {
        ...
        systemMessage.animate({ height: '0px' }, 'slow', undefined, function() {
          $jq(this).hide();
        });
    ...
    function ajaxGet(ajaxPanel, $url, settings, callback) {
      ...
      $jq.ajax({
        ...
        beforeSend: function(xhr) {
          if (!settings.noLoadImg) {
            setLoading(ajaxPanel);
          }
          xhr.requestURL = $url;
        },
        success: function(data) {
    ...
    function tooltipInit() {
      ...
      if ((window.location.pathname || '').match('/tools/genome/jbrowse'))
        return;
    ...
    function searchInit() {
        ...
        $jq.extend(proto, {
          ...
          _renderItem: function(ul, item) {
            return $jq('<li></li>')
              ...
              .append(
                $jq('<a></a>')[this.options.html ? 'html' : 'text'](
      ...
      searchBox.autocomplete({
        source: function(request, response) {
          lastXhr = $jq.getJSON(
            '/search/autocomplete/' + cur_search_type,
            request,
            function(data, status, xhr) {
              if (xhr === lastXhr) {
                data.forEach(function(dat) {
                  var speciesHtml =
                    dat.taxonomy && dat.taxonomy.species
                      ? '<span class="species">' +
                        dat.taxonomy.genus +
                        ' ' +
                        dat.taxonomy.species +
                        '</span>'
                      : '';
                  const {
                  ...
                  var other_names =
                    autocomplete_keyword_all.sort()[0] ||
                    autocomplete_all.join(' ');
                  dat.labelHtml = `
<span class="autocomplete-item-wrapper">
  <span>${dat.label}
  <span style="font-size:x-small;">${
    other_names.replace(/<[^>]*>/g, '') === dat.label ? '' : other_names
  } ${speciesHtml}</spn></span>
</span>`;
                });
                response(data);
              }
    ...
    function search(box) {
      if (!box) {
        ...
        cur_search_type = cur_search_type || 'all';
      }
      var f = $jq('#' + box).val();
      if (f === 'search...' || !f) {
        f = '';
      ...
      location.href =
        ...
        '/' +
        (f === '' ? '*' : f) +
        (cur_search_species_type ? '?species=' + cur_search_species_type : '');
    }
    ...
    function formatExpand(div) {
      ...
      for (
        var i = -1, el, l = expands.size();
        (el = expands.eq(++i)) && i < l;

      ) {
        if (el.height() > 35) {
          el.html(
            '<div class="text-min-expand">' +
              el.html() +
              '</div><div class="more"><div class="ui-icon ui-icon-triangle-1-s"></div></div>'
          ).click(function() {
            var container = $jq(this),
              txt = container.children('.text-min-expand');
            txt
              .animate({ height: txt.height() < 40 ? '100%' : '2.4em' })
              .css('max-height', 'none')
              .next()
              .toggleClass('open')
              .children()
    ...
    function SearchResult(q, type, species, widget, nostar, t, container) {
      var query = decodeURI(q),
        page = 1.0,
        total = t,
        resultDiv = container.find(
          (widget ? '.' + widget + '-widget ' : '') + '.load-results'
        ),
        queryList = query
          ? query.replace(/[,\.\*]|%22|%27/g, ' ').split(' ')
          : [];
      ...
      if (total > 10 || !total) {
        if (container.find('.lazyload-widget').size() > 0) {
          Scrolling.search();
        }
    ...
    function loadResults(url) {
      ...
      loadcount = 0;
      if (!allSearch.hasClass('references')) scrollToTop();
      return false;
    }
    ...
    function allResults(type, species, query, widget) {
      var url_search_base = '/search/' + type + '/' + query,
        url =
          url_search_base + '/1?inline=1' + (species && '&species=' + species),
          ...
          dl.load(dl.attr('href'), function() {
            ...
            if (resultCount < 500) {
              ...
              searchSummary.find('.dl-format a').each(function() {
                ...
                var dl_params = $jq.param({ species: species, format: format });
                var dl_url =
                  url_search_base + '/all' + (dl_params && '?' + dl_params);
              ...
              searchSummary
                .find('.dl-format-list')
                .html(
                  '<li  style="height:auto">Too many results to download. Please use our <a href="ftp://ftp.wormbase.org/pub/wormbase/" target="_blank">FTP</a> site.</li>'
                );
    ...
    function searchFilter(searchSummary, curr) {
      ...
      searchSummary.find('.load-results').click(function() {
        ...
        searchSummary
          .find(".ui-selected:not('#current-ref')")
          .removeClass('ui-selected');
    ...
    function reloadWidget(widget_name, noLoad, url) {
      ...
      if (con.text().length > 4)
        ajaxGet(
          con,
          url || $jq('#nav-' + widget_name).attr('href'),
          { noLoadImg: noLoad, dataType: 'html' },
          function() {
            checkSearch(con);
          }
        );
    ...
    function addWidgetEffects(widget_container) {
      widget_container.find('div.module-min').hover(
        ...
        function() {
          var button = $jq(this);
          button
            .removeClass('ui-icon-circle-triangle-s ui-icon-circle-triangle-e')
            .addClass(
              button.hasClass('show')
      ...
      widget_container.find('div.module-close').hover(function() {
    ...
    var Layout = (function() {
      var sColumns = false,
        ...
        maxWidth =
          location.pathname === '/' || location.pathname === '/me' ? 900 : 1300; //home page? allow narrower columns
        ...
        (function() {
          var instance = this,
            ...
            list = navigation
              .find('.module-load')
              .map(function() {
                return this.getAttribute('wname');
              })
              .get();
          this.wl = { list: list };
          return this.wl;
      ...
      function resize() {
        if (
          sColumns !==
          (sColumns = document.documentElement.clientWidth < maxWidth)
        ) {
          sColumns ? columns(100, 100) : readHash();
          if ((multCol = $jq('#column-dropdown').find('.multCol')))
            multCol.toggleClass('ui-state-disabled');
        }
        if (
          body.hasClass('table-columns') &&
          title.size() > 0 &&
          wHolder.children('.left').width() +
            wHolder.children('.right').width() >
            Math.ceil(
              wHolder
                .children('.left')
                .parent()
          ...
          function(data) {
            ...
            for (i = 0; i < len; i++) {
              ...
              if (node.nodeName === 'data') {
                location.hash = node.attributes.getNamedItem(
                  'lstring'
                ).nodeValue;
              }
            }
      ...
      function newLayout(layout) {
        updateLayout(layout, undefined, function() {
          $jq('.list-layouts').load(
            '/rest/layout_list/' +
              $jq('.list-layouts').data('class') +
              '?section=' +
              $jq('.list-layouts').data('section'),
      ...
      function updateURLHash(left, right, leftWidth, minimized, sidebar) {
        var l = $jq.map(left, function(i) {
            return getWidgetID(i);
          }),
          r = $jq.map(right, function(i) {
            return getWidgetID(i);
          }),
          m = $jq.map(minimized, function(i) {
            return getWidgetID(i);
          }),
          ret =
            l.join('') +
        ...
        if (
          location.hash &&
          decodeURI(location.hash).match(/^[#](.*)$/)[1] !== ret
        ) {
          reloadLayout++;
        }
        location.hash = ret;
      ...
      function readHash() {
        if (reloadLayout === 0) {
          ...
          if (l) {
            l = $jq.map(l.split(''), function(i) {
              return getWidgetName(i);
            });
          }
          if (r) {
            r = $jq.map(r.split(''), function(i) {
              return getWidgetName(i);
            });
          }
          if (m) {
            m = $jq.map(m.split(''), function(i) {
              return getWidgetName(i);
            });
          }
      ...
      function openAllWidgets() {
        var hash = '',
          wlen = $jq('#navigation')
            .find('li.module-load:not(.tools,.me,.toggle)')
            .size();
        if (widgetList.list.length === 0) {
          return;
        }
        for (i = 0; i < wlen; i++) {
      ...
      function readLayout(holder) {
        var left = holder
          ...
          minimized = holder
            .find('.visible .widget-container.minimized')
            .parent()
            .map(function() {
              return this.id;
            })
      ...
      function getLeftWidth(holder) {
        var leftWidth =
          sColumns && location.hash
            ? decodeURI(location.hash)
                .match(/^[#](.*)$/)[1]
                .split('-')[2] * 10
            : (parseFloat(holder.children('.left').outerWidth()) /
                parseFloat(holder.outerWidth())) *
              100;
      ...
      function resetLayout(
        ...
        for (
          var widget = 0, l = leftList ? leftList.length : 0;
          widget < l;
          widget++
        ) {
        ...
        for (
          var widget = 0, l = rightList ? rightList.length : 0;
          widget < l;
          widget++
        ) {
        ...
        for (
          var widget = 0, l = minimized ? minimized.length : 0;
          widget < l;
          widget++
        ) {
    ...
    var Scrolling = (function() {
      ...
      function goToAnchor(anchor) {
        var elem = document.getElementById(anchor),
          scroll = isScrolledIntoView(elem)
            ? undefined
            : $jq(elem).offset().top - system_message - 10;
        if (scroll) {
      ...
      var sidebarScroll = (function() {
        ...
        var loop = function() {
          sidebarUl
            .stop()
            .animate(
              { scrollTop: sidebarUl.scrollTop() + 100 },
              1000,
              'linear',
              loop
            );
        ...
        var updateScrollState = function() {
          if (
            sidebarUl.scrollTop() <
            sidebarUl.prop('scrollHeight') - sidebarUl.height() - 5
          ) {
      ...
      function sidebarMove() {
        if (!sidebar) return;
        if (sidebar.offset()) {
          var objSmallerThanWindow =
              sidebar.outerHeight() < $window.height() - system_message ||
              sidebar.find('.closed').size() > 0,
            scrollTop = $window.scrollTop(),
            maxScroll =
              $jq(document).height() -
              (sidebar.outerHeight() +
                $jq('#footer').outerHeight() +
                system_message +
                20); //the 20 is for padding before footer
          ...
          if (isStatic === 0) {
            if (scrollTop >= offset && scrollTop <= maxScroll) {
              sidebar
                .stop(false, true)
                .css('position', 'fixed')
                .css('top', system_message);
              isStatic = 1;
            } else if (scrollTop > maxScroll) {
              sidebar
                .stop(false, true)
                .css('position', 'fixed')
                .css('top', system_message - (scrollTop - maxScroll));
              //isStatic = 1;
            } else {
              //resetSidebar();
            }
    ...
    function validate_fields(
      ...
      if (email.val() === '' && (!wbemail || wbemail.val() === '')) {
        ...
        email.val() &&
        validate_email(email.val(), 'Not a valid email address!') === false
      ) {
        email.focus().addClass('ui-state-error');
        email
          .closest('#issues-new')
    ...
    var comment = {
      ...
      submit: function(cm) {
        ...
        var box = $jq(
          '<div class="comment-box"><a href="/me">' +
            name +
            '</a> ' +
            content +
            '<br /><span id="fade">just now</span></div>'
        );
    ...
    var issue = {
      ...
      submit: function(is) {
        ...
        if (!validate_fields(email)) return;
        if (!content) {
        ...
        content =
          $jq('<textarea/>')
            .html(content.val())
            .val() || content.html(); //convert encoded html error report to html
        content = content.replace(/^\s+/gm, ''); // avoid problematic leading spaces for github
        ...
        content +=
          dc && $jq.trim(dc.val())
            ? '<p>What were you doing? <br />&nbsp;&nbsp;' + dc.val() + '</p>'
            : '';
        ...
        $jq.ajax({
          ...
          dataType: 'json',
          data: {
            title: feed.find('#issue-title option:selected').val(),
            content: content,
            name: name.val(),
            email: email.val(),
            url: url || issue.url,
        ...
        addNewLink.show();
        message.append(
          "<p><h2 style='color:rgb(95, 112, 137);'>Thank you for helping WormBase!</h2></p><p>The WormBase helpdesk will get back to you shortly. You will recieve an email confirmation momentarily. Please email <a href='mailto:help@wormbase.org'>help@wormbase.org</a> if you have any concerns.</p>"
        );
        return false;
      },
      ...
      delete_widget: function(widget_id) {
        if (confirm('are you sure you want to delete this widget?')) {
          $jq.ajax({
            ...
            success: function(data) {
              $jq('#nav-static-widget-' + widget_id)
                .click()
                .hide();
            },
            error: function(xhr, status, error) {
              $jq('li#static-widget-' + widget_id)
                .find('.content')
                .html(ajaxError(xhr));
            },
          });
    ...
    function historyOn(action, value, callback) {
      if (action === 'get') {
        ...
        $jq('.user-history')
          .add('#user_history-content')
          .html(
            "<div><span id='fade'>Please wait, updating your history preferences</span></div>"
          );
      }
    }
    ...
    function loadRSS(id, url) {
      ...
      $jq.get(url, function(xml) {
        ...
        var txt = []
          .concat('<div id="results"><ul>', entriesHtml, '</ul></div>')
          .join('');
        container.html(txt);
    ...
    var openid = {
      ...
      signinWormMine: function(provider) {
        ...
        var mineUrlBase =
          'https://www.wormbase.org/tools/wormmine/openid?provider=%s';
        var mineUrl;
      ...
      var drawCallback = function(settings) {
        ...
        api
          ...
          .each(function(rowData, i) {
            ...
            if (last !== group) {
              var summary_row = row_summarize
                ? row_summarize(rowData)
                : '<td>' + group + '</td>';
              $jq(rows)
                .eq(i)
                .before(
    ...
    function loadCytoscapeForPersonLineage(
      elementsDirect,
      elementsFull,
      thisPerson
    ) {
    ...
    function setupCytoscapePersonLineage(
      elementsDirect,
      elementsFull,
      thisPerson
    ) {
    ...
    function setupCytoscapeInteraction(
      dataUrl,
      focusNodeId,
      initialState = {}
    ) {
      import('./components/InteractionGraph').then((module) => {
    ...
    function setupCyPersonLineage(elementsDirect, elementsFull, thisPerson) {
      ...
      $jq('#toggleToFullView').on('click', function() {
        ...
        if (jsonExportSaveFull === '') {
          // if there is no previous save for Full, render from elements
      ...
      $jq('#view_png_button').on('click', function() {
        var png64 = cyPersonLineageAll.png({
          full: true,
          maxWidth: 8000,
          maxHeight: 8000,
          bg: 'white',
      ...
      for (i = 0; i < arrayOfInputs.length; i++) {
        if (arrayOfInputs[i].type == 'checkbox') {
          ...
          arrayOfInputs[i].onclick = function(event) {
            updateFromCheckboxes();
          }; // when checkbox is clicked, update based on checkboxes
        }
      }

      function updateFromCheckboxes() {
        // update cytoscape graph based on state of checkboxes
        cyPersonLineageAll.elements('edge').hide(); // hide all edges
        var nodeHash = new Object(); // put nodes here that have an edge that shows
        for (j = 0; j < arrayOfCheckboxes.length; j++) {
          // for each checkbox
          if (arrayOfCheckboxes[j].checked) {
            ...
            for (k = 0; k < arrayEdges.length; k++) {
              // for each edge
              if (arrayEdges[k].data().role == role) {
                // if the edge has the checked role
        ...
        cyPersonLineageAll.elements('node').filter(function(i, ele) {
          // filter on nodes
          if (nodeHash.hasOwnProperty(ele.id())) {
            // if the node is is in the hash of nodes to show
    ...
    function setupCytoscapeGoGraph(elementId, wbId) {
      loadOntologyGraph().then(({ GeneOntologyGraph }) => {
        return ReactDOM.render(
          <Root>
            <GeneOntologyGraph focusTermId={wbId} />
          </Root>,
    ...
    function setupCytoscapeLifestageGraph(elementId, wbId) {
      loadOntologyGraph().then(({ LifeStageOntologyGraph }) => {
        return ReactDOM.render(
          <Root>
            <LifeStageOntologyGraph focusTermId={wbId} />
          </Root>,
          document.getElementById(elementId)
        );
      });
    ...
    function setupCytoscapeAnatomyGraph(elementId, wbId) {
      loadOntologyGraph().then(({ AnatomyOntologyGraph }) => {
        return ReactDOM.render(
          <Root>
            <AnatomyOntologyGraph focusTermId={wbId} />
          </Root>,
          document.getElementById(elementId)
        );
      });
    ...
    function setupCytoscapeDiseaseGraph(elementId, wbId) {
      loadOntologyGraph().then(({ DiseaseOntologyGraph }) => {
        return ReactDOM.render(
          <Root>
            <DiseaseOntologyGraph focusTermId={wbId} />
          </Root>,
          document.getElementById(elementId)
        );
      });
    ...
    function setupCytoscapePhenotypeGraph(elementId, wbId) {
      loadOntologyGraph().then(({ PhenotypeOntologyGraph }) => {
        return ReactDOM.render(
          <Root>
            <PhenotypeOntologyGraph focusTermId={wbId} />
          </Root>,
          document.getElementById(elementId)
        );
      });
    ...
    function loadOntologyGraph() {
      return import('./components/OntologyGraph');
    }
    ...
    function setupCyInteractionViewer(data, types, clazz) {
      ...
      var typeColorMapper = (function() {
        ...
        for (var i = 0; i < types.length; i++) {
          ...
          map[types[i]] = types[i] == 'Predicted' ? '#999' : edgeColor[i];
        }
        return map;
      })();
        ...
        ready: function() {
          ...
          legend.find('input:checkbox').click(function() {
            ...
            if (this.name == 'phenotypeToggle') {
              if (this.checked) {
                legend
                  .find('input:checkbox[name="phenotype"]')
                  .prop('checked', true);
              } else {
                legend
                  .find('input:checkbox[name="phenotype"]')
      ...
      function resetChecked() {
        legend.find('input:checkbox').map(function() {
          ...
          if (t.attr('name') == 'type') {
            t.prop(
              'checked',
              clazz === 'Predicted' ? true : !t.val().match('Predicted')
            );
      ...
      function updateEdgeFilter() {
        ...
        var edgeTypes = legend
          ...
          .get();
        var edgeDirs = legend
          .find('input[name="direction"]:checked')
          .map(function() {
            return this.getAttribute('value');
          })
          .get();
        var edgePhens = legend
          .find('input[name="phenotype"]:checked')
          .map(function() {
            return this.getAttribute('value');
          })
          .get();
        ...
        var nearbyExists =
          legend.find('input[name=nearby]').size() > 0 ? true : false;
        var nearbyChecked =
          legend.find('input[name=nearby]:checked').size() > 0 ? true : false;
        ...
        cy.elements('edge')
          .filter(function(i, ele) {
            ...
            if (
              (!ele.data().phenotype ||
                $jq.inArray(ele.data().phenotype, edgePhens) > -1) &&
              (!ele.data().type ||
                $jq.inArray(ele.data().type, edgeTypes) > -1) &&
              (!ele.data().direction ||
                $jq.inArray(ele.data().direction, edgeDirs) > -1) &&
      ...
      function updateNodeFilter() {
        ...
        // Interactor types
        var intTypes = legend
          .find('input[name=nodes]:not(:checked)')
          .map(function() {
            return this.getAttribute('value');
          })
          .get();
    ...
    var FpkmPlots = (function() {
      ...
      function makeFpkmSummaryPlot(container, experiments, data) {
        ...
        var CATEGORICAL_STAGES = CATEGORICAL_STAGES_PARTITIONED.reduce(function(
          prev,
          stagesInPartition
        ) {
        ...
        function update() {
          ...
          if (cleanData.length < 1) {
            $jq(container).html(
              '<span class="caveat-emptor" style="position:relative;top:1.5em;">There is no FPKM expression data for this gene from the selected modENCODE libraries.</span>'
            );
            return;
          }
          ...
          var sortedByLifeStage = Object.keys(lifeStage2Data)
            .slice()
          ...
          container.highcharts({
            ...
            subtitle: {
              text:
                'This shows the FPKM expression values of PolyA+ and Ribozero modENCODE libraries across life-stages. The bars show the median value of the libraries plotted. Other modENCODE libraries which were made using other protocols or which are from a particular tissue or attack by a pathogen have been excluded.',
              style: {
                color: LABEL_COLOR,
                'font-size': '10px',
              },
        ...
        function preprocess_data(experiments, data) {
          var selectedData = data.filter(function(d) {
            ...
            return (
              (d.project_info.id === 'SRP000401' ||
                d.project_info.id === 'RNASeq_Study.SRP000401') &&
          ...
          var summarized = Object.keys(libraries).map(function(libraryId) {
            ...
            var median = ss.median(
              replicates.map(function(d) {
        ...
        function plotBands() {
          var bands = STAGES_TYPES.map(function(typeName, index) {
            var palette = ['rgba(68, 170, 213, 0.1)', 'rgba(0, 0, 0, 0)'];
            var from, to;
      ...
      function makeFpkmBoxPlot(container, projects, data) {
        ...
        function setupMenu(projects) {
          var listItems = Object.keys(projects)
            .sort(function(a, b) {
          ...
          var menuHtml = []
            .concat('<ul class="plot-menu">', listItems, '</ul>')
            .join('');
        ...
        function lifeStageValue(lifeStageName) {
          ...
          if (/early embryo/i.test(lifeStageName)) {
            ...
            var cellCount = parseInt(
              /(\d+)-cell embryo/i.exec(lifeStageName)[1]
            );
            return cellCount / LARGE_NUMBER;
          } else if (/(\d+) min post first-cleavage/i.test(lifeStageName)) {
            var minCount = parseInt(
              /(\d+) min post first-cleavage/i.exec(lifeStageName)[1]
            );
            ...
            var dayCount = parseInt(
              /(\d+)-day(s?) post-L4 adult/i.exec(lifeStageName)[1]
            );
        ...
        function updatePlot(projectID) {
          ...
          var categories = Object.keys(seriesDataRaw).sort(function(
            lifeStageA,
            lifeStageB
          ) {
          ...
          var minPoints = 2;
          var boxplotData = allData.reduce(function(
            boxData,
            categoryData,
            index
          ) {
            if (categoryData.length < minPoints) {
              ...
              var boxParams = [].concat(
                index, // x
                boxSummaryStat(categoryData)
              ); // boxplot stats
          ...
          var boxplotOtherPoints = allData.reduce(function(
            pointsData,
            categoryData,
            index
          ) {
          ...
          var allPoints = allData.reduce(function(
            pointsData,
            categoryData,
            categoryIndex
          ) {
        ...
        function updateDescription(projectID) {
          ...
          function getLink(tag) {
            ...
            return (
              '<strong><a href="' + link + '">' + tag.label + '</a></strong>'
            );
          }

          newDescription =
            getLink(projectObject.tag) +
            '<div class="text-min"><p>' +
        ...
        function boxSummaryStat(dataArray) {
          ...
          var whiskerBottom = Math.min.apply(
            null,
            dataArray.filter(function(dataValue) {
              return dataValue >= lowerBound;
            })
          );
          var whiskerTop = Math.max.apply(
            null,
            dataArray.filter(function(dataValue) {
    ...
    function renderWidget(data, elementId, widgetName) {
      ...
      ReactDOM.render(
        <WidgetComponent data={data} pageInfo={pageInfo} />,
        document.getElementById(elementId)
      );
    }

    function renderGORibbon(data, elementId) {
      import('./components/GORibbon').then((module) => {
    ...
    function renderVariationSequence(data, elementId) {
      import('./components/Sequence').then((module) => {
                ...
                <div>
                  {['wildtype', 'mutant'].map((type) => {
                    const { sequence, features } =
                      strand === '+'
                        ? data[type]['positive_strand']
                        : data[type]['negative_strand'];
                    ...
                    return (
                      <SequenceCard
                        ...
                        title={`${type} ${
                          data['public_name']
                        }, with ${flankLength}bp flanks, shown on (${strand}) strand`}
                        downloadFileName={`${data['public_name']}__${type}__${
                          strand === '+' ? 'positive' : 'negative'
                        }_strand__with_${flankLength}bp_flanks.fasta`}
                        sequence={sequence}
                        features={features}
                        featureLabelMap={{
        ...
        ReactDOM.render(
          <div>
            {['wildtype', 'mutant'].map((type) => {
              ...
              const features = Object.keys(data['protein_effects'] || {})
                .map((k) => data['protein_effects'][k])
                .map(({ position, description, ...proteinEffect }) => ({
                  start:
                    parseInt(proteinEffect[`${type}_start`]) ||
                    parseInt(position),
                  stop:
                    parseInt(proteinEffect[`${type}_stop`]) ||
                    parseInt(position),
                  type: 'variation',
                  effect: description,
                }))
      ...
      import('./components/Sequence').then((module) => {
        ...
        ReactDOM.render(
          <StrandSelect initialStrand={data.reported_on_strand}>
            {({ strand }) => {
              const { sequence, features } =
                strand === '-' ? negative_strand : positive_strand;
              return (
                <SequenceCard
                  key={sequence}
      ...
      import('./components/Sequence').then((module) => {
        ...
        ReactDOM.render(
          <TranscriptSequencesCard
            ...
            unsplicedSequenceContextWithPadding={
              unspliced_sequence_context_with_padding.data
            }
    ...
    function renderInteractorVennDiagram(data, elementId) {
      import('./components/InteractorVennDiagram').then((module) => {
    ...
    function renderInferredPathways(data = {}, elementId) {
      const { geneId } = data;
      import('@wormbase/third-party-data-api/lib').then(
        ({ ThirdPartyDataProvider, ReactomePathwayList }) => {
          window.React3 = require('react');
          console.log(`Same react? ${window.React2 === React}`);
          console.log(`Same react? ${require('react') === React}`);
          console.log(`Same react? ${require('react') === window.React2}`);
          // console.log(`Same react? ${require('../../node_modules/react' === window.React2)}`);
          return ReactDOM.render(
    ...
    var Plugin = (function() {
      var pluginsLoaded = new Array(),
        ...
        loading = false,
        pScripts = {
          highlight: '/js/jquery/plugins/jquery.highlight-1.1.js',
          dataTables:
            '/js/jquery/plugins/dataTables/media/js/jquery.dataTables.min.js',
          colorbox:
            '/js/jquery/plugins/colorbox/colorbox/jquery.colorbox-min.js',
          ...
          tabletools:
            '/js/jquery/plugins/tabletools/media/js/TableTools.min.js',
          placeholder: '/js/jquery/plugins/jquery.placeholder.min.js',
          cytoscape_js:
            '/js/jquery/plugins/cytoscapejs/cytoscape_min/2.5.0/cytoscape.min.js',
          cytoscape_js_arbor:
            '/js/jquery/plugins/cytoscapejs/cytoscape_arbor/1.1.2/cytoscape-arbor.js',
          cytoscape_js_dagre:
            '/js/jquery/plugins/cytoscapejs/cytoscape_dagre/1.1.2/cytoscape-dagre.js',
          qtip: '/js/jquery/plugins/qtip2/2.2.0/jquery.qtip.min.js',
          cytoscape_js_qtip:
            '/js/jquery/plugins/cytoscapejs/cytoscape_js_qtip/2.2.5/cytoscape-qtip.js',
          ...
          simple_statistics:
            '/js/simple-statistics/1.0.1/simple_statistics.min.js', // statistics library in JS
          icheck: '/js/jquery/plugins/icheck-1.0.2/icheck.min.js',
        },
        pStyle = {
          //dataTables: "/js/jquery/plugins/dataTables/media/css/demo_table.css",  // loading with require;
      ...
      function addToCallLater(name, toCallFunction) {
        if (typeof callLater[name] === 'undefined') {
          callLater[name] = new Array();
        }
